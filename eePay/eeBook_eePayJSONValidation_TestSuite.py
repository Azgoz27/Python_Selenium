...
class EEBKG_EEPAY_ValidateJSON(TestFixturesUIBaseClass):
    ...
    def test_01_LoadAndCompareTestCards(self):
        ...
        for type in FOP:
            ...
            if set(foundCards) == set(list(testCards)):
                self.logger.info("SUCCESS: Displayed {} cards match the test cards.".format(type))
            else:
                self.logger.info("FAIL: Displayed {} cards DO NOT match the test cards.".format(type))
    ...
    def test_02_inputCardsAndValidatesImage(self):
        ...
        for type in FOP:
            self.selectFOP(type)
            testCards = self.determineTestCards(type)
            self.logger.info("TESTING {} CARDS:".format(type.upper()))
            ...
            for key, value in testCards.items():
                ...
                if key in cardImage:
                    self.logger.info("SUCCESS: Displayed card image matches the test {} card.".format(key))
                else:
                    self.logger.info("FAIL: Displayed card image DOES NOT match the test {} cards.".format(key))
                    self.chromeTakeFullScreenshot(self.driver, screenshotFolder="./screenshots/",
                                             filePrefix=self._testMethodName)
                    self.fail("Test case: %s failed, check logs" % self._testMethodName)
            ...
            for key, value in testCards.items():
                ...
                if self.driver.find_elements_by_xpath('//*[@id="currency"]'):
                    ...
                    for currency in currencyOptions:
                        ...
                        self.logger.info("Found {} currency option with price total:{}.".format(currency.text, price.text))
                    currencySelector.find_elements_by_tag_name("option")[1].click()
                    self.logger.info("Currency selector loaded for {}.".format(key))
                else:
                    self.logger.info("WARNING: Currency selector NOT loaded for {}.".format(key))
